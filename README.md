TopTalk网站,照片 壁纸 绘画和涂鸦爱好者书写情感的地方.已经在阿里云服务器上进行外网测试 http://47.98.240.27

使用SpringBoot搭建的项目,使用Mysql数据库,Spring Data JPA,Security,运行环境为Linux,以jar包的方式运行,目前网页图片储存在本地,受带宽限制,网页图片加载会非常慢,后续更新计划引用七牛云来储存,

另外有window本地版本,区别在于文件夹路径不同,另外网页上的图片会在本地储存;

网站简介:

开发动机:由于以前在联系Python爬虫的时候经常用一个音乐网站来测试爬取图片(www.luoo.com),感觉此网站前端页面设计得很简洁漂亮,板块全面而且功能不杂乱,虽然此网站是一个在线听歌的网站,但是板块却很适合改造成分享壁纸的网站项目,于是我将页面全部另存为本地文件后准备改造成自己的壁纸分享网站;

网站分为四个大板块:
     专辑板块 专题壁纸分享板块(例如巴黎专辑,樱花专辑,建筑专辑) ; 
     随笔板块 单张壁纸分享板块 (例如微软bing的每日一张壁纸推荐) , 
     故事板块 有故事的系列壁纸分享板块(例如游记,电影截图影评) , 
     活动板块 组织网友活动的板块(例如旅游,聚会,照相等)(计划改成推荐书刊杂志的板块);

网站功能:
  


注意事项,项目中url地址中带/upload的是映射本地文件夹/usr/projectdev/images/upload,所有图片会存在此文件夹内子文件夹中,可以先测试投稿页面再查看项目运行效果,否则部分页面板式会混乱或报错;
