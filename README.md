...
写这段话的时候是2018年10月31日,目前只有我和上帝看得懂这个网站项目是怎么运行的,代码写得实在太乱了,我自己都改得头晕,估计下个月只有上帝能看得懂了,所以今天决定把这个项目代码全部推翻重构;
...
##  网站功能详情:
  ...
  <h4>4:修改头像和账号信息的功能:</h4>
   ...
   为了保证用户账号的安全性,密码采用了BCrypt加密方式,不会保存原始密码;
...
##  后续版本更新计划:
  ...
  4:(已实现)将用户密码进行加密,开发用户修改密码的功能;
  
  5:开发网站消息功能;

  6:开发网站角色权限管理功能(管理员角色权限,专栏作者角色权限,普通用户角色权限,封禁用户角色权限);
  
  7:开发邮件提醒功能(在投稿成功时发送邮件通知管理员审核);
  
##  **********开 发 记 录 *********

2018/10/31:

1 : 我觉得这个项目的代码写得太乱了,全部推翻重构   !-_-
...
2 : 为了方便以后测试投稿板块,增加了根据系统自动选择是否开启七牛云的功能,默认情况下,win系统会关闭七牛云,所有投稿会保存本地路径到数据库中,阿里云linux生产环境会开启七牛云;
(可以通过properties文件来手动开启或禁用自动配置);S
...
1：更换了网站标题为PhotoLike；
2：因为数据库现在保存的是加密的密码字段，所以原数据库的几名网友的测试账号被移除了；
